...
const displayPokemonDetails = async (pokemon) => {
  ...
  const desiredLanguage = "pt";
  let overview = "Os jogadores podem escolher os movimentos e itens que os Pokémon podem aprender e segurar nas batalhas.";
  let genus = "Os Pokémon ficam mais fortes e aprendem novos movimentos à medida que as batalhas avançam";
  ...
  <div>Gênero: <b><i class="fa-solid fa-mars" style="color: #1f71ff;"></i>${male}  <i class="fa-solid fa-venus" style="color: #ff5c74;"></i>${female}</b></div>
  <span>Habilidades: <b>${abilities.join(", ")}</b></span>
  <span>Taxa de captura: <b>${catchRate} (${((catchRate / 255) * 100).toFixed(
    2
  )}% chance)</b></span>
  <span>Amizade Básica: <b>${friendship} (${
    friendship < 50 ? "mais baixo" : friendship < 100 ? "normal" : "mais alto"
  })</b></span>
  <span>Exp Base: <b>${pokemon[0].base_experience}</b></span>
  <span>Taxa de crescimento: <b>${pokemon[1].growth_rate.name}</b></span>
  <span>Tipos de ovos: <b>${eggGroups.join(", ")}</b></span>
